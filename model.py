...
class CognitiveDiagnosisModel(nn.Module):
        ...
        class WrappedModule(nn.Module):
            ...
            def forward(self, emb1, emb2):
                ...
                if isinstance(result, tuple) and len(result) == 2:
                    return result
                # 优化：使用 new_zeros 避免设备转移开销
                return result, emb1.new_zeros(())
        return WrappedModule(module)

    def _init_weights(self):
        """初始化模型权重（Embedding层已在LightGCN中初始化）"""
    ...
    def forward(self, stu_ids, exer_ids, cpt_ids_padded, cpt_mask, labels,
                adj_correct_se, adj_wrong_se, adj_correct_sc, adj_wrong_sc):
        """
        前向传播 (保持原始训练语义，GCN + Fusion 每 batch 计算并参与梯度更新)
        
        优化点：cpt_ids_padded 和 cpt_mask 由 collate_fn 预处理完成，避免 Python 循环
        
        Args:
            stu_ids: LongTensor [B]
            exer_ids: LongTensor [B]
            cpt_ids_padded: LongTensor [B, L] - 已 padding 的概念 ID
            cpt_mask: BoolTensor [B, L] - True 表示有效位置
            labels: FloatTensor [B]
            adj_*: 稀疏邻接矩阵
        """
        # GCN 计算 (每 batch 计算，参与梯度更新)
        ...
        # 对比学习 - 概念 (使用 mask 获取有效概念)
        valid_cpt_ids = cpt_ids_padded[cpt_mask]
        unique_cpt_ids = valid_cpt_ids.unique()
        
        if len(unique_cpt_ids) > 1:
            if self.use_supervised_contrastive and labels is not None:
                cpt_labels = torch.ones(len(unique_cpt_ids), device=stu_ids.device)
                contrastive_loss_cpt = self.contrastive_concept(
                    cpt_emb_correct[unique_cpt_ids], cpt_emb_wrong[unique_cpt_ids], cpt_labels
                )
            else:
                contrastive_loss_cpt = self.contrastive_concept(
                    cpt_emb_correct[unique_cpt_ids], cpt_emb_wrong[unique_cpt_ids]
                )
        else:
            contrastive_loss_cpt = stu_ids.new_zeros((), dtype=torch.float)

        # 获取 batch 嵌入
        batch_stu_emb = stu_emb_final[stu_ids]
        batch_exer_emb = exer_emb_final[exer_ids]
        
        # 优化：向量化获取概念嵌入 (使用 collate_fn 预处理的 padded tensor)
        batch_cpt_embs = cpt_emb_final[cpt_ids_padded]  # [B, L, D]

        # 诊断预测
        predictions = self.diagnosis_head(batch_stu_emb, batch_exer_emb, batch_cpt_embs, cpt_mask)
